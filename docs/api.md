...
| `/dellink <key>` | Админ | Запрос на удаление связки (с подтверждением). Команда доступна только админам |
...
| `links` | Кнопка `❌ Нет` в диалоге удаления (карточка), «🔙 К списку», «🔙 Назад» в карточке | Возвращает к списку связок |
