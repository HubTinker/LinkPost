...
- **Экран (навигация)** — сообщение, которое пользователь может переключать кнопками. Оно становится источником `message_callback` → **редактируется на месте**.
...
| Экран без клавиатуры (нет кнопок) | Исключение: новое сообщение (edit без клавиатуры оставил бы старые кнопки; см. §5.4) |
...
2. После неудачного edit удаляем ровно тот target, который пытались редактировать. Никогда не редактируем/удаляем другой экран из KV.
...
Сохранение `status_message_id` происходит строго до вызова цепочки — гонки с завершением нет.
...
`message_callback`-обновление содержит `message_id` у `update.message` (предположение; код уже использует `update.message.recipient.chat_id`, но наличие `message_id` на реальном трафике не подтверждено). Если `message_id` не приходит — фича работает через фолбэк `nav_msg:{chatId}` (§3, строка 2). После деплоя проверить по логам `renderScreen` (WARN-строки покажут частоту фолбэков).
