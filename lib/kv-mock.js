...
export const kv = {
  ...
  async *scanIterator ({ match = '*', count = 100 } = {}) {
    const matched = []
    for (const key of [...store.keys(), ...sets.keys(), ...zsets.keys()]) {
      if (matchPattern(match, key)) {
        matched.push(key)
      }
    }
    for (let i = 0; i < matched.length; i += count) {
      yield matched.slice(i, i + count)
    }
    console.log('[KV-MOCK] scanIterator', match, '\u2192', matched.length)
  },
