...
export async function sendBroadcastMessage (userId, broadcast) {
  if (userId == null) {
    console.error('[API] sendBroadcastMessage: userId is required')
    throw new Error(`userId is required, got ${userId}`)
  }
  ...
  return request('POST', `/messages?user_id=${userId}`, payload)
}

export function extractMessageId (response) {
  // Реальный формат MAX API: ответ содержит message.body.mid (строка mid.00000000167b21dd019ffa40a8604932),
  // а не message_id. Проверено на живом webhook (2026-08-13).
  return response?.message?.body?.mid ?? response?.message_id ?? null
}
