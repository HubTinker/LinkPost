...
export async function updateBroadcast (id, patch) {
  ...
  const allowed = ['title', 'text', 'images', 'buttons', 'format', 'status', 'scheduled_at', '_images_done', '_buttons_done', 'created_by_chat_id']
  const safePatch = {}
...
export async function getScheduledBroadcasts () {
  const kv = await getKv()
  // @vercel/kv v2 имеет zrangebyscore; v3 (на @upstash/redis) — только zrange с byScore
  let ids
  if (typeof kv.zrangebyscore === 'function') {
    ids = await kv.zrangebyscore(BR_SCHEDULED_ZSET, 0, Date.now())
  } else {
    ids = await kv.zrange(BR_SCHEDULED_ZSET, 0, Date.now(), { byScore: true })
  }
  if (!ids.length) return []
  const values = await Promise.all(ids.map(id => kv.get(`${BR_PREFIX}${id}`)))
